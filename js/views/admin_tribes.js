// js/views/admin_tribes.js
// Patch 5.8.1 — Edition des points de catégorie (Tribu)
...
function entityPanel(list, entity, label, onRefresh){
  ...
  title.innerHTML = `<b>${entity.name || '(sans nom)'}</b> <span class="muted small">— $Tribu</span>`;
...
export function renderAdminTribes(){
  const S = State.get();
  const list = S.tribes || (S.tribes = []);
  console.log('[JDR] Tribu view loaded, items:', list.length);
  return makeList('Tribu', list);
}

// Extra aliases to maximize compatibility with existing imports
export const renderTribes = renderAdminTribes;
export const renderTribeTab = renderAdminTribes;
export const renderAdminTribe = renderAdminTribes;
...
if (typeof window !== 'undefined') {
  ...
  window.JDRViews['Tribu'] = { render: renderAdminTribes };
  console.log('[JDR] Global view alias registered for Tribu');
}
