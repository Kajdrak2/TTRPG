// js/views/admintribes.js — Build A14 (Tribus) — Nom + Catégories + Stats + Ressources avec bonus/malus (accordéon)
import { el } from '../core/ui.js';
import * as State from '../core/state.js';

function N(v){ return (isFinite(+v)? Math.trunc(+v) : 0); }
function arr(a){ return Array.isArray(a)? a : []; }
function ensureCol(S){ S.tribes = arr(S.tribes); return S.tribes; }
...
function listAllowedResources(S, entName){
  ...
  (S.resources||[]).forEach(r=>{
    ...
    if(scope==='tribus' && entName && linked.indexOf(entName)>=0) out.push(nm);
  });
  return out;
}
...
function rowEntity(S, ent, refreshAll){
  ...
  const left = el('div'); const b = document.createElement('b'); b.textContent = ent.name || 'Tribu'; left.appendChild(b);
  ...
  (function(){
    ...
    nameI.oninput = ()=>{ ent.name = nameI.value||''; State.save(S); b.textContent = ent.name || 'Tribu'; };
...
export function renderAdminTribus(S){
  const root = el('div');
  const pnl = el('div','panel'); const head = el('div','list-item'); head.innerHTML = '<div><b>Tribus</b></div>'; pnl.appendChild(head);
  ...
  function refresh(){
    ...
    const add = el('div','list-item small');
    const l = el('div'); l.textContent='Ajouter tribu'; add.appendChild(l);
    const r = el('div');
    ...
    addB.onclick = ()=>{ const nm=(nameI.value||'').trim(); if(!nm) return; ensureCol(S).push({ id:'trib_'+Math.random().toString(36).slice(2,9), name:nm, catMods:{}, mods:{ stats:{}, resources:{} } }); State.save(S); refresh(); };
    r.append(nameI, addB); add.appendChild(r); list.appendChild(add);
  }
  refresh();
  return root;
}
export default renderAdminTribus;
