// js/views/admin_races.js
// Patch 5.8.1 — Edition des points de catégorie (Race)
...
function entityPanel(list, entity, label, onRefresh){
  ...
  title.innerHTML = `<b>${entity.name || '(sans nom)'}</b> <span class="muted small">— $Race</span>`;
...
export function renderAdminRaces(){
  const S = State.get();
  const list = S.races || (S.races = []);
  console.log('[JDR] Race view loaded, items:', list.length);
  return makeList('Race', list);
}

// Extra aliases to maximize compatibility with existing imports
export const renderRaces = renderAdminRaces;
export const renderRaceTab = renderAdminRaces;
export const renderAdminRace = renderAdminRaces;
export default renderAdminRaces;

// Optional global fallback (if some code expects globals)
if (typeof window !== 'undefined') {
  window.JDRViews = window.JDRViews || {};
  window.JDRViews['Race'] = { render: renderAdminRaces };
  console.log('[JDR] Global view alias registered for Race');
}
