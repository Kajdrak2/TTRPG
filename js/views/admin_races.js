// js/views/admin_races.js — Build A12 (Races)
import { el } from '../core/ui.js';
import { get, save } from '../core/state.js';

function ensureArray(a){ return Array.isArray(a)?a:[]; }
function ensureCol(S){ S.races = ensureArray(S.races); return S.races; }
...
function listResFor(S, entName){
  ...
  for(var i=0;i<R.length;i++){ var r=R[i]; var name=r.name||r.id; if(!name) continue;
    ...
    else if(scope==='races' && 'races'==='races'){ if(Array.isArray(r.linked) && r.linked.indexOf(entName)>=0) out.push(name); }
    else if(scope==='tribus' && 'races'==='tribes'){ if(Array.isArray(r.linked) && r.linked.indexOf(entName)>=0) out.push(name); }
    else if(scope==='classes' && 'races'==='classes'){ if(Array.isArray(r.linked) && r.linked.indexOf(entName)>=0) out.push(name); }
  }
  return out;
}
...
function row(S, ent, onRefresh){
  ...
  var hl = el('div'); var b=document.createElement('b'); b.textContent = ent.name||'Race'; hl.appendChild(b);
  ...
  (function(){
    ...
    inp.oninput=function(){ ent.name=inp.value||''; save(S); b.textContent=ent.name||'Race'; };
...
export function renderAdminRaces(S0){
  var S = get();
  ensureCol(S);
  var root = el('div');
  var panel = el('div','panel'); var head=el('div','list-item'); var hdL=el('div'); hdL.innerHTML='<b>Races</b>'; head.appendChild(hdL); panel.appendChild(head);
  ...
  function refresh(){
    ...
    var left = el('div'); left.textContent='Ajouter race'; add.appendChild(left);
    var right = el('div');
    ...
    addB.onclick=function(){ var nm=(nameI.value||'').trim(); if(!nm) return; ensureCol(S).push({ id:'race_'+Math.random().toString(36).slice(2,9), name:nm, catDelta:0, mods:{ stats:{}, cats:{}, resources:{} } }); save(S); refresh(); };
...
export default renderAdminRaces;
